...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, language }) => {
  ...
  // Extraemos la descripción traducida en base a la clave
  const translatedDescription = translations[language].projects.items[project.descriptionKey as keyof typeof translations['en']['projects']['items']];
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        {/* Usamos el texto dinámico para la descripción */}
        <p className="text-text-secondary-light dark:text-text-secondary text-sm flex-grow mb-4">{translatedDescription}</p>
        <div className="flex flex-wrap gap-2 mb-6">
          {project.tags.map((tag, tagIndex) => (
            <span key={tagIndex} className="bg-slate-100 dark:bg-slate-800 text-xs font-semibold text-text-secondary-light dark:text-text-secondary px-3 py-1 rounded-full">
              {tag}
            </span>
          ))}
        </div>
